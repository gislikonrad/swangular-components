...
export class OAuthService {    private _callbackUrl: string;
    ...
    canSignIn(): boolean {
      if(!this.callbackReady && !this._errorReported) {
        ...
        this._errorService.setError('Cannot sign in for secure api request. The auth callback component has not been put into the app component template. Add the <api-auth-callback></api-auth-callback> to your app component template.');
      }
